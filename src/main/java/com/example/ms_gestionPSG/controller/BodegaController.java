package com.example.ms_gestionPSG.controller;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.HttpStatus;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseStatus;
import org.springframework.web.bind.annotation.RestController;

import com.example.ms_gestionPSG.model.Bodega;
import com.example.ms_gestionPSG.service.IBodegaService;

@RestController
@RequestMapping("/api/bodegas")
public class BodegaController {

	@Autowired
	private IBodegaService boservice;
	
	@GetMapping
	@ResponseStatus(code = HttpStatus.OK)
	public List<Bodega> obtenerBodegas(){
		return boservice.listarBodegas();
		
	}
	@GetMapping("/{id}")
	@ResponseStatus(code = HttpStatus.OK)
	public Bodega obtenerBodegaId(@PathVariable("id") Integer id){
		return boservice.obtenerBodegaId(id);
		
	}
}
